...
#***************compute female percentage*************
    females=0
    females_num=len(item['borrowers'])
    for i in item['borrowers']:
        if i['gender']=='F':
            females+=1
    borrower_page.append(round(float(females)*100/float(females_num),2))

#*************compute funded_date******************
    try:
        funded_date=item['funded_date'][:-1]
        borrower_page.append(funded_date[:10])
    except:
        borrower_page.append('XXXX')
#*****************************************************
